...
document.querySelectorAll('.footer-bottom p').forEach((footerCopyright) => {
    footerCopyright.textContent = `© ${new Date().getFullYear()} A.P ASOCIADOS C.A. Todos los derechos reservados.`;
});
...
const initHeroSlideshow = () => {
    ...
    if (slogan) {
        requestAnimationFrame(() => slogan.classList.add('is-revealed'));
    }

    if (images.length < 2 || prefersReducedMotion || isLowPowerDevice) return;
